...
const InvoiceDropdownSize = ({ onEditItem, cellData }) => {
    const [selected, setSelected] = useState(
        sizes.find((size) => size.name === cellData.value) || sizes[0]
    );

    useEffect(() => {
        // When the cellData.value changes, update the selected value
        setSelected(sizes.find((size) => size.name === cellData.value) || sizes[0]);
    }, [cellData.value]);
...
export default InvoiceDropdownSize;
